...
class ConfigBuilder {
    ...
    private fun parse(tagStr : String) : Statement{
        ...
        return Statement(group,orientation,type,prop, parseArgs(lang.group("args"),"id") != null,parseArgs(lang.group("args"),"required") != null,token = (parseArgs(lang.group("args"),"token"))?.get(0),default =  parseArgs(lang.group("args"),"defaultValue"))
    }
    ...
    companion object{
        ...
        private val langPattern : Pattern = Pattern.compile("(?P<end>end)?\\s*(?P<repetition>each|single)\\s*(?P<orientation>row|column)?\\s*(?P<type>string|number|((?P<boolean>boolean)\\(((?P<true>[^\\s]*)\\s*as\\s*(true|false))\\s*,\\s*((?P<false>[^\\s]*)\\s*as\\s*(true|false))\\))|(enum\\((?P<enumeral>(.+)(,\\s*.+)*)\\)))\\s*(?P<group>[^\\s]*)\\s*((as)\\s*(?P<prop>[^\\s]*))?\\s*(?P<args>.*)?")
